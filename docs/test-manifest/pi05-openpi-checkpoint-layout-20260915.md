...
- PASS：Gemini H20 上最终组合回归为 `5 passed, 3 warnings in 35.93s`；warnings 仅为
  单进程 DCP 未初始化 distributed 的提示。
- PASS：真实 `pi05_base_pytorch` 两卡 FSDP2 测试为
  `1 passed in 195.31s (0:03:15)`，覆盖 812-key OpenPI root strict load、FP32 master
  权重保存、AdamW DCP probe 恢复和第二次完整保存。
