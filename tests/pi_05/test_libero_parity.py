...
def _compare_float(name: str, actual: np.ndarray, expected: np.ndarray) -> None:
    ...
    dice = (
        0.0
        if denominator == 0
        else 1 - 2 * np.multiply(actual64, expected64).sum() / denominator
    )
    ...
    np.testing.assert_allclose(actual64, expected64, rtol=1e-3, atol=1e-3, err_msg=name)
...
def test_libero_policy_matches_openpi_pytorch() -> None:
    ...
    with np.load(Path(GOLDEN), allow_pickle=False) as golden:
        ...
        _compare_float("raw_actions_32d", raw_actions.float().cpu().numpy(), golden["raw_actions"])
        decoded = policy.infer(raw, noise=noise)["actions"]
        _compare_float("decoded_actions_7d", decoded, golden["decoded_actions"])
